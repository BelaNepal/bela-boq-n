...
export async function generateBOQPdfFromFormData(formData: BOQFormData) {
  ...
  try {
    ...
    const drawTableRow = (cols: string[], isHeader: boolean = false, isBold: boolean = false) => {
      const colWidths = [10, 50, 47, 13, 18, 20, 22]; // Adjusted to match A4 printable width (180mm)
      const lineHeight = 4;
      ...
      wrappedCols.forEach((textArr, index) => {
        ...
        textArr.forEach((line) => {
          ...
          // Increased gap from right for better alignment visual
          const textX = index >= 3 ? xPos + colWidths[index] - 4 : xPos + 2;
    ...
    for (const sec of sections) {
      ...
      for (const sub of sec.items) {
        ...
        // Amount
        doc.text(`NRS ${formattedTotal}`, pageWidth - marginRight - 4, yPosition, { align: "right" });
    ...
    const addFooter = () => {
      ...
      doc.setFont("helvetica", "normal");
      doc.text("Visit us at www.belanepal.com.np", marginLeft, pageHeight - 19);
      doc.text("Email us: info@belanepal.com.np", pageWidth / 2, pageHeight - 19, { align: "center" });
      doc.setFont("helvetica", "bold");
      doc.setTextColor(239, 126, 30);
      doc.text(`Page ${pageNum}`, pageWidth - marginRight, pageHeight - 15, { align: "right" });
    };
    const finalPageCount = doc.getNumberOfPages();
    for (let p = 1; p <= finalPageCount; p++) {
      doc.setPage(p);
      pageNum = p;
      // addFooter(p, finalPageCount); // Refactor addFooter to accept args or just inline it here for simplicity
      doc.setDrawColor(239, 126, 30);
      doc.setLineWidth(0.5);
      doc.line(marginLeft, pageHeight - 25, pageWidth - marginRight, pageHeight - 25);
      doc.setFontSize(7);
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(239, 126, 30);
      doc.text(`Page ${p} of ${finalPageCount}`, pageWidth - marginRight, pageHeight - 15, { align: "right" });
    }
...
export async function generateQuotationPdf(formData: BOQFormData, quotationDetails?: { quotationNumber: string, quotationDate: string, validityDays: number }) {
  // Reuse the logic or create a similar structure. For now, adapting the existing one with a different Title and extra fields.
  // Ideally we would refactor to a shared internal function, but for safety/minimal diff, we will duplicate/adjust.
  ...
  try {
    ...
    if (quotationDetails) {
      doc.setFontSize(9);
      doc.setTextColor(30, 45, 77);
      doc.text(`Quotation No: ${quotationDetails.quotationNumber}`, marginLeft, yPosition);
      doc.text(`Date: ${quotationDetails.quotationDate}`, pageWidth - marginRight, yPosition, { align: "right" });
      yPosition += 6;
    }
    ...
    const drawTableRow = (cols: string[], isHeader: boolean = false, isBold: boolean = false) => {
      // Total width = 180 (A4 210mm - 15mm left - 15mm right)
      const colWidths = [10, 50, 47, 13, 18, 20, 22]; // Adjusted to match A4 printable width (180mm)
      const lineHeight = 4;
    ...
    for (const sec of sections) {
      ...
      for (const sub of sec.items) {
        ...
        doc.text(sub.title, 15, yPosition);
        yPosition += 7;
        drawTableRow(["S.N.", "Item Description", "Specification", "Unit", "Quantity", "Rate", "Amount"], true);
        const sectionTotal = sub.data.reduce((sum, item) => sum + parseNumber((item as any).amount), 0);
        sectionGrandTotal += sectionTotal;
        const titleMatch = String(sub.title).trim().match(/^([A-Za-z])/);
        const sectionSuffix = titleMatch ? titleMatch[1].toLowerCase() : "a";
    ...
    const projectName = (formData.projectInfo.projectName || "").replace(/[^a-z0-9]/gi, "_");
    const dateStr = new Date().toISOString().split("T")[0];
    doc.save(`Quotation_${projectName}_${dateStr}.pdf`);
  } catch (e) {
    throw e;
  }
}
